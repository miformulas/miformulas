...
with sync_playwright() as p:
    ...
    check("build stamp 260908", pg.locator("#build").inner_text().strip() == "260908")
    ...
    check("manifest linked", pg.evaluate("document.querySelector('link[rel=manifest]')?.getAttribute('href')") == "manifest.webmanifest")
    check("install link hidden until the browser offers it", not pg.locator("#btnInstall").is_visible())
    # simulate the browser's install offer
    pg.evaluate("""() => { const e = new Event('beforeinstallprompt'); e.prompt = () => { window.__prompted = true; }; e.userChoice = Promise.resolve({outcome:'accepted'}); window.dispatchEvent(e); }""")
    pg.wait_for_timeout(200)
    check("install link shown after beforeinstallprompt", pg.locator("#btnInstall").is_visible())
    ...
    check("save to file: cancel keeps browser storage", not ps.locator("#storageHint").evaluate("e => e.hidden") and ps.evaluate("!HANDLE"))
    ps.click("#btnSettings"); ps.wait_for_timeout(400)
    check("settings in browser storage: Save-to-file offered", ps.locator("#setToFile").is_visible())
    ps.click("#dlgCancel"); ps.wait_for_timeout(200)
    ps.click("#storageHintSave"); ps.wait_for_timeout(400); ps.click("#dlgOk"); ps.wait_for_timeout(1200)
    check("save to file: written and remembered", ps.evaluate("!!HANDLE") and ps.evaluate("idb.get('fileHandle').then(h => h && h.name)") == "miformulas-data.json" and "formulas" in (ps.evaluate("localStorage.getItem('fakefile')") or ""))
    check("save to file: bar hidden, state Saved", ps.locator("#storageHint").evaluate("e => e.hidden") and ps.locator("#saveState").inner_text().startswith("Saved"))
    ps.click("#btnSettings"); ps.wait_for_timeout(400)
    check("settings after save: no Save-to-file, Forget offered", not ps.locator("#setToFile").is_visible() and ps.locator("#setForget").is_visible())
    ps.click("#dlgCancel"); ps.wait_for_timeout(200)
    ps.evaluate("idb.set('fileHandle', null)"); ps.evaluate("localStorage.removeItem('fakefile')"); ps.evaluate("idb.set('demoData', null)")
