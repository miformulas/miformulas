...
This manual describes build 260916b. The build number of the copy you are using is shown next to the name in the top-left corner of the app, and in the Help bar; on a phone the header leaves it out, so read it there.
...
**How do I get my materials in? They are in a spreadsheet.** Save the sheet as a CSV and use **Import materials inventory from CSV…** on the Welcome page; the app shows your columns next to its fields and you match them on screen. Only the name is required. Section 18 has the details, and **Download a template** shows the columns the app uses itself.
