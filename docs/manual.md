...
This manual describes build 260917c. The build number of the copy you are using is shown next to the name in the top-left corner of the app, and in the Help bar; on a phone the header leaves it out, so read it there.
...
Another presentation of the same recipe is a version too: the formula at 20 % instead of 15 %, with the costly material taken from the 10 % dilution instead of the pure one (make a new version and change the dilutions with **Preserve rel % and exchange solvent**, section 13), or made up as a 44 g batch instead of 100 g (Batch scaling, section 14, or simply the target total on the weighing sheet, which rescales the print without touching the formula). Give such a version a label with the **pencil** ("20%", "soap", "44gr") so the list tells them apart. Earlier builds had a separate notion of "variations" for this; it went in build 260915b, because versions do the same with less to learn. A data file that still holds variations loads with a warning: they are left in the file untouched, but not shown.
...
Below the table sit **Batch scaling** (section 14), **Notes**, the **Trial log** (section 15), the **Categories** panel and the **IFRA check** (section 18), and **Mark as prepared** for the stock ledger (section 19).
